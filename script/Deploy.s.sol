...
/// Deploys the agency with every single-stock aggregator publishing on
/// Robinhood chain as of September 2026. No owner, so this list is the
/// edition: a new census means a new edition.
contract Deploy is Script {
    function run() external {
        address[] memory fs = new address[](29);
        string[] memory ns = new string[](29);
        ...
        ns[28] = "USAR";
        vm.startBroadcast();
        AAA agency = new AAA(fs, ns);
        vm.stopBroadcast();
        console.log("AAA deployed at", address(agency));
    }
}
